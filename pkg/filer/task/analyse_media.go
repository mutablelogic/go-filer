...
var (
	// Leave lowercase
	audiobookGenres = []string{"spoken word", "audiobook", "audio book", "book", "books & spoken"}
)

///////////////////////////////////////////////////////////////////////////////
// PUBLIC METHODS

func (t *taskrunner) AnalyseMedia(ctx context.Context, object *schema.Object) error {
	var errs error
	...
	ref.Log(ctx).With("media", media).Debugf(ctx, "Updated media with audio/video properties")
...
func (t *taskrunner) createMedia(ctx context.Context, object *schema.Object, r *ffmpeg.Reader) (*schema.Media, error) {
	...
	// Set artist, album, composer, genre
	...
	if track, exists := meta.GetMetaString("track", "iTunes_CDDB_TrackNumber", "episode", "episode_id"); exists {
		...
		if count, err := strconv.ParseUint(countStr, 10, 64); err == nil {
			meta.Count = types.Uint64Ptr(count)
		}
	}
	...
	// Timestamp
	if timestamp, exists := meta.GetMetaString("creation_time"); exists {
		// Parse like this: 2003-05-24T11:43:25.000000Z
		if t, err := time.Parse(time.RFC3339, timestamp); err == nil {
			meta.Ts = types.TimePtr(t)
		}
	}

	// Types
	if meta.Type == "audio" && meta.Genre != nil && slices.Contains(audiobookGenres, strings.ToLower(types.PtrString(meta.Genre))) {
		meta.Type = "audiobook"
	}
