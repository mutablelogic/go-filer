...
func Test_ManagerMem_CreateObject(t *testing.T) {
	...
	t.Run("create simple object", func(t *testing.T) {
		...
		obj, err := mgr.CreateObject(ctx, "testbucket", schema.CreateObjectRequest{
			Path:        "/created.txt",
			Body:        strings.NewReader(content),
	...
	t.Run("overwrite existing object", func(t *testing.T) {
		...
		// Create initial
		_, err := mgr.CreateObject(ctx, "testbucket", schema.CreateObjectRequest{
			Path: "/overwrite.txt",
...
func Test_ManagerMem_GetObject(t *testing.T) {
	...
	t.Run("get existing object metadata", func(t *testing.T) {
		...
		obj, err := mgr.GetObject(ctx, "testbucket", schema.GetObjectRequest{Path: "/getme.txt"})
		assert.NoError(err)
		assert.Equal("testbucket", obj.Name)
...
func Test_ManagerMem_ListObjects(t *testing.T) {
	...
	t.Run("list root non-recursive", func(t *testing.T) {
		...
		resp, err := mgr.ListObjects(ctx, "testbucket", schema.ListObjectsRequest{
			Path:      "/",
			Recursive: false,
...
func Test_ManagerMem_DeleteObject(t *testing.T) {
	...
	t.Run("delete existing object", func(t *testing.T) {
		...
		// Delete it
		obj, err := mgr.DeleteObject(ctx, "testbucket", schema.DeleteObjectRequest{Path: "/deleteme.txt"})
		assert.NoError(err)
		assert.Equal("testbucket", obj.Name)
		...
		// Verify it's gone
		_, err = mgr.GetObject(ctx, "testbucket", schema.GetObjectRequest{Path: "/deleteme.txt"})
		assert.Error(err)
		assert.Contains(err.Error(), "not found")
	})
	...
	t.Run("delete non-existent object", func(t *testing.T) {
		...
		_, err := mgr.DeleteObject(ctx, "testbucket", schema.DeleteObjectRequest{Path: "/notfound.txt"})
		assert.Error(err)
		assert.Contains(err.Error(), "not found")
	})
	...
	t.Run("delete nested object", func(t *testing.T) {
		...
		// Delete it
		_, err = mgr.DeleteObject(ctx, "testbucket", schema.DeleteObjectRequest{Path: "/nested/deep/file.txt"})
		assert.NoError(err)

		// Verify it's gone
		_, err = mgr.GetObject(ctx, "testbucket", schema.GetObjectRequest{Path: "/nested/deep/file.txt"})
		assert.Error(err)
	})
}
...
func Test_ManagerMem_RoutesToCorrectBackend(t *testing.T) {
	...
	// Create in bucket1
	_, err = mgr.CreateObject(ctx, "bucket1", schema.CreateObjectRequest{
		Path: "/test1.txt",
	...
	// Create in bucket2
	_, err = mgr.CreateObject(ctx, "bucket2", schema.CreateObjectRequest{
		Path: "/test2.txt",
	...
	obj1, err := mgr.GetObject(ctx, "bucket1", schema.GetObjectRequest{Path: "/test1.txt"})
	assert.NoError(err)
	assert.Equal("bucket1", obj1.Name)
	assert.Equal("/test1.txt", obj1.Path)

	// Verify object in bucket2
	obj2, err := mgr.GetObject(ctx, "bucket2", schema.GetObjectRequest{Path: "/test2.txt"})
	assert.NoError(err)
	assert.Equal("bucket2", obj2.Name)
	assert.Equal("/test2.txt", obj2.Path)

	// Verify cross-bucket isolation
	_, err = mgr.GetObject(ctx, "bucket1", schema.GetObjectRequest{Path: "/test2.txt"})
	assert.Error(err)
	_, err = mgr.GetObject(ctx, "bucket2", schema.GetObjectRequest{Path: "/test1.txt"})
	assert.Error(err)
}
...
func Test_ManagerMem_FullWorkflow(t *testing.T) {
	...
	createdObj, err := mgr.CreateObject(ctx, "testbucket", schema.CreateObjectRequest{
	...
	gotObj, err := mgr.GetObject(ctx, "testbucket", schema.GetObjectRequest{Path: "/workflow/test.txt"})
	assert.NoError(err)
	...
	// 4. List objects
	listResp, err := mgr.ListObjects(ctx, "testbucket", schema.ListObjectsRequest{Path: "/workflow/", Limit: schema.MaxListLimit})
	assert.NoError(err)
	assert.Len(listResp.Body, 1)

	// 5. Delete object
	deletedObj, err := mgr.DeleteObject(ctx, "testbucket", schema.DeleteObjectRequest{Path: "/workflow/test.txt"})
	assert.NoError(err)
	assert.Equal("testbucket", deletedObj.Name)
	assert.Equal("/workflow/test.txt", deletedObj.Path)

	// 6. Verify object is gone
	_, err = mgr.GetObject(ctx, "testbucket", schema.GetObjectRequest{Path: "/workflow/test.txt"})
	assert.Error(err)
...
func Test_ManagerMem_EdgeCases(t *testing.T) {
	...
	t.Run("special characters in path", func(t *testing.T) {
		...
		// Should be retrievable
		_, err = mgr.GetObject(ctx, "testbucket", schema.GetObjectRequest{Path: "/path with spaces/file-name_v1.2.txt"})
		assert.NoError(err)
	})
