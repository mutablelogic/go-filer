...
func init() {
	registry.RegisterExtractor(new(imageextractor))
}
...
func (e *imageextractor) ExtractMetadata(ctx context.Context, path string) ([]schema.MetadataKV, error) {
	...
	// Append the metadata
	metadata := schema.AppendMetadataKV([]schema.MetadataKV{}, extractor.ImageFormat, format)
	metadata = schema.AppendMetadataKV(metadata, extractor.ImageWidth, cfg.Width)
	metadata = schema.AppendMetadataKV(metadata, extractor.ImageHeight, cfg.Height)
	...
	metadata = schema.AppendMetadataKV(metadata, extractor.ImageTitle, summarizer.Title)
	metadata = schema.AppendMetadataKV(metadata, extractor.ImageSummary, summarizer.Summary)
	if len(summarizer.Keywords) > 0 {
		metadata = schema.AppendMetadataKV(metadata, extractor.ImageTags, summarizer.Keywords)
	}
